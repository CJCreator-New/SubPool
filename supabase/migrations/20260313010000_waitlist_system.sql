...
BEGIN
    ...
    INTO v_already_waiting, v_waitlist_pos
    FROM pool_waitlist
...
DECLARE
    v_waitlist_pos INTEGER;
BEGIN
    SELECT "position" INTO v_waitlist_pos
    FROM pool_waitlist
    WHERE pool_id = p_pool_id 
      AND user_id = p_user_id 
      AND status = 'waiting';
    
    RETURN v_waitlist_pos;
END;
$$ LANGUAGE plpgsql;
